...
from cre.core import numba_type_map
from cre.caching import source_in_cache, source_to_cache
...
def gen_source_standard_imports():
    ...
    imports += "from cre.numbalizer import _assert_map\n"
...
#     if(typ == 'f8'):
#         s += "from cre.aot_template_funcs import backtrace_selection_f8\n"
...
#     else:
#         s += "from cre.aot_template_funcs import backtrace_selection\n"
...
#     s += "import cre.aot_template_funcs\n"
...
#     s += ind + "return cre.aot_template_funcs.insert_record(record_type, history, depth, op_uid, btsr_flat, btsr_shape, arg_types, vmap)\n\n"
...
def gen_apply_inf_history_aot_funcs(typ,hsh,custom_type=False,ind='   '):
    typ_v = "NB_"+typ if custom_type else typ
    s = "from cre.aot_template_funcs import gen_inf_history_aot_funcs\n"
    s += "gen_inf_history_aot_funcs(cc, '{}', {})\n\n".format(typ, typ_v)
    return s

# def gen_apply_knowledge_store_aot_funcs(typ,hsh,custom_type=False,ind='   '):
#     typ_v = "NB_"+typ if custom_type else typ    
#     s = "from cre.kb import gen_knowledge_store_aot_funcs\n"
